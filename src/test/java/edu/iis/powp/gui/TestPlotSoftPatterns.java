...
import edu.iis.powp.adapter.MagicPlotterAdapter;
import edu.iis.powp.app.Application;
import edu.iis.powp.app.Context;
import edu.iis.powp.app.DriverManager;
import edu.iis.powp.appext.ApplicationWithDrawer;
import edu.iis.powp.events.predefine.SelectChangeVisibleOptionListener;
import edu.iis.powp.events.predefine.SelectLinePlotterOptionListener;
...
public class TestPlotSoftPatterns {
	...
	private static void setupPresetTests(Context context) {
		...
		context.addTest(SelectTestFigureOptionListener.FIGURE_JOE_1, selectTestFigureOptionListener);
		context.addTest(SelectTestFigureOptionListener.FIGURE_JOE_2, selectTestFigureOptionListener);
		context.addTest(SelectTestFigureOptionListener.FIGURE_JANE, selectTestFigureOptionListener);
	...
	private static void setupDrivers(Context context) {
		...
		IPlotter plotter = new MagicPlotterAdapter();
		context.addDriver("Buggy Simulator", plotter);

		// Add line plotter adapter
		IPlotter linePlotter = new LinePlotterAdapter();
		context.addDriver("Special Line Simulator", linePlotter);

		// Add support for line plotter configuration
		ActionListener linePlotterListener = new SelectLinePlotterOptionListener();
		context.addComponentMenuElement(DrawPanelController.class, SelectLinePlotterOptionListener.BASIC_LINE,
				linePlotterListener);
		context.addComponentMenuElement(DrawPanelController.class, SelectLinePlotterOptionListener.SPECIAL_LINE,
				linePlotterListener);
		context.addComponentMenuElement(DrawPanelController.class, SelectLinePlotterOptionListener.DOTTED_LINE,
				linePlotterListener);
